...
func test_pointer_math() {
    ...
    test("pointer comparison works - 2", () => {
        var arr = [10, 20, 30, 40, 50];
        var ptr1 = &raw arr[0] + 2;
        var ptr2 = &raw arr[0] + 2;
        return ptr1 == ptr2;
    })
    test("pointer comparison works - 3", () => {
        var arr = [10, 20, 30, 40, 50];
        var ptr1 = &raw arr[0] + 4;
        var ptr2 = &raw arr[0];
        return ptr2 < ptr1;
    })
    ...
    test("taking address of pointer works", () => {
        ...
        const thing = *take_addr_of_pointer(&raw mut ptr);
        return thing == 33
    })
    ...
    test("passing a struct pointer as typealias succeeds", () => {
        var p = PMC22 { a : 43, b : 87 }
        return send_a_struct_ptr(&raw p)
    })
    test("index operator works on pointer values", () => {
        var arr = [ 10, 20, 30 ]
        var first = &raw mut arr[0]
        return first[2] == 30
    })
