...
/* x64 CONTEXT (subset with general regs) */
...
@extern @dllimport @stdcall public func ImageNtHeader(Base : *void) : *mut IMAGE_NT_HEADERS;

var g_crashing : int = 0
var g_on_crash : () => void = null

/* load modules and register symbols */
func load_process_modules_and_register_symbols(process : HANDLE) {
    ...
    var modules = malloc(cbNeeded as usize) as *mut *mut void;
    if (!modules) return;
...
func prepare_symbol_info_buffer(buffer : *mut char, si : *mut _SYMBOL_INFO) {
    memset(buffer as *mut void, 0, sizeof(SYMBOL_INFO) + MAX_SYM_NAME);
    si.MaxNameLen = MAX_SYM_NAME;
    si.SizeOfStruct = (sizeof(SYMBOL_INFO) + (MAX_SYM_NAME - 1) * sizeof(char)) as ULONG;
}

/* Resolve single address and print function + file:line */
func resolve_and_print_win(addr : DWORD64, idx : uint) {
    var proc : HANDLE = GetCurrentProcess();
    var buf_sym : [sizeof(SYMBOL_INFO) + MAX_SYM_NAME]char;
    var si : *mut _SYMBOL_INFO = buf_sym as *mut _SYMBOL_INFO;
    prepare_symbol_info_buffer(buf_sym, si);
    var disp : DWORD64 = 0;
    if (SymFromAddr(proc, addr, &disp, si)) {
        fprintf(stderr, "%02u: %s + 0x%llx [0x%llx]", idx, si.Name, disp as ubigint, addr as ubigint);
    } else {
        var err : DWORD = GetLastError();
        var modBase : DWORD64 = SymGetModuleBase64(proc, addr);
        fprintf(stderr, "%02u: [0x%llx] (SymFromAddr failed err=%lu, module_base=0x%llx)", idx, addr as ubigint, err as ulong, modBase as ubigint);
    }

    var line : IMAGEHLP_LINE64;
    memset(&line as *mut void, 0, sizeof(line));
...
/* The main Windows exception handler function */
@stdcall
func CrashHandlerException(ep_void : *mut void) : LONG {
    ...
    memset(&frame as *mut void, 0, sizeof(frame));
    ...
    if(def.x86_64) {
        ...
        frame.AddrPC.Offset = 0 as DWORD64;
...
/* Install handler via SetUnhandledExceptionFilter */
