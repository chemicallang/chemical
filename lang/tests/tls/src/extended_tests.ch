...
public func INT_gcm_aes128_encrypt(env : &mut TestEnv) {
    ...
    unsafe var aad : [8]u8; test_random_bytes(&raw mut aad[0], 8)
    unsafe var pt : [32]u8; test_random_bytes(&raw mut pt[0], 32)
    ...
    unsafe var aad_hex : [17]char; test_bytes_to_hex(&raw aad[0], 8, &raw mut aad_hex[0])
    unsafe var pt_hex : [65]char; test_bytes_to_hex(&raw pt[0], 32, &raw mut pt_hex[0])
    ...
    unsafe var py_ct : [32]u8; unsafe var py_tag : [16]u8
...
public func INT_gcm_empty_aad(env : &mut TestEnv) {
    unsafe var key : [16]u8; test_random_bytes(&raw mut key[0], 16)
    unsafe var iv : [12]u8; test_random_bytes(&raw mut iv[0], 12)
    unsafe var pt : [16]u8; test_random_bytes(&raw mut pt[0], 16)

    unsafe var key_hex : [33]char; test_bytes_to_hex(&raw key[0], 16, &raw mut key_hex[0])
    unsafe var iv_hex : [25]char; test_bytes_to_hex(&raw iv[0], 12, &raw mut iv_hex[0])
    unsafe var pt_hex : [33]char; test_bytes_to_hex(&raw pt[0], 16, &raw mut pt_hex[0])
    ...
    unsafe var py_ct : [16]u8; unsafe var py_tag : [16]u8
...
public func INT_gcm_long_aad(env : &mut TestEnv) {
    unsafe var key : [16]u8; test_random_bytes(&raw mut key[0], 16)
    unsafe var iv : [12]u8; test_random_bytes(&raw mut iv[0], 12)
    unsafe var aad : [1024]u8; test_random_bytes(&raw mut aad[0], 1024)
    unsafe var pt : [16]u8; test_random_bytes(&raw mut pt[0], 16)

    unsafe var key_hex : [33]char; test_bytes_to_hex(&raw key[0], 16, &raw mut key_hex[0])
    unsafe var iv_hex : [25]char; test_bytes_to_hex(&raw iv[0], 12, &raw mut iv_hex[0])
    unsafe var aad_hex : [2049]char; test_bytes_to_hex(&raw aad[0], 1024, &raw mut aad_hex[0])
    unsafe var pt_hex : [33]char; test_bytes_to_hex(&raw pt[0], 16, &raw mut pt_hex[0])
...
public func INT_aes_cbc_128_roundtrip(env : &mut TestEnv) {
    ...
    unsafe var pt : [64]u8; test_random_bytes(&raw mut pt[0], 64)

    unsafe var ctx : AESContext
    aes_init(&raw mut ctx)
    var ret = aes_setkey_enc(&raw mut ctx, &raw key[0], 16)
    if(ret < 0) { env.error("aes_setkey_enc failed"); return } else {}

    unsafe var enc_out : [80]u8
    unsafe var enc_iv : [16]u8; var ii : size_t = 0; while(ii < 16) { enc_iv[ii] = iv[ii]; ii += 1 }
    ...
    unsafe var dec_iv : [16]u8; ii = 0; while(ii < 16) { dec_iv[ii] = iv[ii]; ii += 1 }
    unsafe var dec_out : [80]u8
...
public func INT_hmac_sha256_two_keys(env : &mut TestEnv) {
    unsafe var key1 : [16]u8; test_random_bytes(&raw mut key1[0], 16)
    unsafe var key2 : [32]u8; test_random_bytes(&raw mut key2[0], 32)
    unsafe var data : [16]u8; test_random_bytes(&raw mut data[0], 16)
    unsafe var d_hex : [33]char; test_bytes_to_hex(&raw data[0], 16, &raw mut d_hex[0])

    unsafe var mac1 : [32]u8; hmac_sha256(&raw key1[0], 16, &raw data[0], 16, &raw mut mac1[0])
    unsafe var mac2 : [32]u8; hmac_sha256(&raw key2[0], 32, &raw data[0], 16, &raw mut mac2[0])

    unsafe var k1_hex : [33]char; test_bytes_to_hex(&raw key1[0], 16, &raw mut k1_hex[0])
    unsafe var k2_hex : [65]char; test_bytes_to_hex(&raw key2[0], 32, &raw mut k2_hex[0])
    ...
    unsafe var py_m1 : [32]u8; unsafe var py_m2 : [32]u8
...
public func INT_sha256_vs_python(env : &mut TestEnv) {
    unsafe var data : [128]u8; test_random_bytes(&raw mut data[0], 128)
    unsafe var data_hex : [257]char; test_bytes_to_hex(&raw data[0], 128, &raw mut data_hex[0])
...
public func INT_sha256_incremental(env : &mut TestEnv) {
    unsafe var part1 : [32]u8; test_random_bytes(&raw mut part1[0], 32)
    unsafe var part2 : [32]u8; test_random_bytes(&raw mut part2[0], 32)
    unsafe var combined : [64]u8
    var ci : size_t = 0
    while(ci < 32) { combined[ci] = part1[ci]; ci += 1 }
    ci = 0; while(ci < 32) { combined[32+ci] = part2[ci]; ci += 1 }

    unsafe var combined_hex : [129]char; test_bytes_to_hex(&raw combined[0], 64, &raw mut combined_hex[0])
...
public func INT_sha256_long_input(env : &mut TestEnv) {
    ...
    unsafe var script : [4096+256]u8; var sp : size_t = 0
    var hdr = "import hashlib\n" as *char; var si : size_t = 0
    while(hdr[si]!=0){script[sp]=hdr[si] as u8; sp+=1; si+=1}
    var l = "h=hashlib.sha256(bytes.fromhex('" as *char; si=0
    while(l[si]!=0){script[sp]=l[si] as u8; sp+=1; si+=1}
...
public func INT_md5_basic_vs_python(env : &mut TestEnv) {
    unsafe var data : [64]u8; test_random_bytes(&raw mut data[0], 64)
    unsafe var data_hex : [129]char; test_bytes_to_hex(&raw data[0], 64, &raw mut data_hex[0])
...
public func INT_md5_incremental(env : &mut TestEnv) {
    unsafe var part1 : [16]u8; test_random_bytes(&raw mut part1[0], 16)
    unsafe var part2 : [16]u8; test_random_bytes(&raw mut part2[0], 16)
    unsafe var part3 : [16]u8; test_random_bytes(&raw mut part3[0], 16)
    unsafe var combined : [48]u8
    var ci : size_t = 0
    ...
    unsafe var combined_hex : [97]char; test_bytes_to_hex(&raw combined[0], 48, &raw mut combined_hex[0])
...
@test public func INT_x25519_keygen(env : &mut TestEnv) {
    unsafe var priv : [32]u8; unsafe var pub : [32]u8
    if(x25519_generate_keypair(&raw mut priv[0], &raw mut pub[0]) < 0){env.error("x25519 keygen");return}else{}
    unsafe var p_hex : [65]char; test_bytes_to_hex(&raw pub[0], 32, &raw mut p_hex[0])
    unsafe var script : [512]u8; var sp : size_t = 0; var si : size_t = 0
...
@test public func INT_x25519_shared_vs_py(env : &mut TestEnv) {
    unsafe var alice_priv : [32]u8; unsafe var alice_pub : [32]u8
    x25519_generate_keypair(&raw mut alice_priv[0], &raw mut alice_pub[0])
    unsafe var ap_hex : [65]char; test_bytes_to_hex(&raw alice_pub[0], 32, &raw mut ap_hex[0])
    unsafe var script : [512]u8; var sp : size_t = 0; var si : size_t = 0
...
@test public func INT_x25519_rfc7748_testvec(env : &mut TestEnv) {
    ...
    unsafe var scalar : [32]u8; scalar[0]=9; var i:size_t=1;while(i<32){scalar[i]=0;i+=1}
    unsafe var u_coord : [32]u8; u_coord[0]=9; i=1;while(i<32){u_coord[i]=0;i+=1}
    unsafe var out : [32]u8; x25519_ladder(&raw mut out[0], &raw scalar[0], &raw u_coord[0])
...
@test public func INT_ecdsa_sign_verify_p256(env : &mut TestEnv) {
    ...
    unsafe var sig : [128]u8; var sig_len : u16 = 128
    ...
    unsafe var pub_key : [65]u8; pub_key[0]=4; var i:size_t=0;while(i<32){pub_key[1+i]=px_hex[i];pub_key[33+i]=py_hex[i];i+=1}
...
@test public func INT_ecdsa_verify_py_sig(env : &mut TestEnv) {
    ...
    unsafe var px_hex : [64]u8; unsafe var py_hex : [64]u8; unsafe var sig_hex : [128]u8
    test_parse_py_hex_label(&raw mut py_out, string_view("PX="), &raw mut px_hex[0], 32)
    test_parse_py_hex_label(&raw mut py_out, string_view("PY="), &raw mut py_hex[0], 32)
...
@test public func INT_ecdsa_sign_py_verify(env : &mut TestEnv) {
    ...
    unsafe var hash : [32]u8; test_random_bytes(&raw mut hash[0], 32)
    unsafe var h_hex : [65]char; test_bytes_to_hex(&raw hash[0], 32, &raw mut h_hex[0])
    unsafe var ctx : ECDSAContext; ecdsa_init(&raw mut ctx); ecdsa_import_privkey(&raw mut ctx, &raw sk_hex[0], 32, TLS_GROUP_SECP256R1 as u16)
    unsafe var sig : [128]u8; var sig_len : u16 = 128
    ecdsa_sign(&raw mut ctx, &raw hash[0], 32, &raw mut sig[0], &raw mut sig_len)
    unsafe var sig_hex : [257]char; test_bytes_to_hex(&raw sig[0], sig_len as size_t, &raw mut sig_hex[0])
...
@test public func INT_ecdsa_p384_pubkey(env : &mut TestEnv) {
    ...
    unsafe var px : [48]u8; unsafe var py : [48]u8
...
@test @test.timeout(60000) public func INT_rsa_encrypt_decrypt(env : &mut TestEnv) {
    ...
    unsafe var n_hex : [512]u8; unsafe var d_hex : [512]u8
    ...
    unsafe var e_bytes : [3]u8; e_bytes[0]=1; e_bytes[1]=0; e_bytes[2]=1
    unsafe var pt : [16]u8; test_random_bytes(&raw mut pt[0], 16)
    unsafe var ctx : RSAContext; rsa_init(&raw mut ctx, 0, 0)
    ...
    unsafe var ctx2 : RSAContext; rsa_init(&raw mut ctx2, 0, 0)
...
@test @test.timeout(60000) public func INT_rsa_sign_verify(env : &mut TestEnv) {
    ...
    unsafe var n_hex : [256]u8; unsafe var sig_hex : [256]u8; unsafe var msg_hex : [64]u8
    var n_len = test_parse_py_hex_label(&raw mut py_out, string_view("N="), &raw mut n_hex[0], 256)
    ...
    unsafe var ctx : RSAContext; rsa_init(&raw mut ctx, 0, 0)
    rsa_import_pubkey(&raw mut ctx, &raw n_hex[0], n_len, &raw e_bytes[0], 3)
    unsafe var digest : [32]u8; sha256_hash(&raw msg_hex[0], msg_len, &raw mut digest[0])
...
@test public func INT_tls12_prf_vs_py(env : &mut TestEnv) {
    unsafe var secret : [32]u8; test_random_bytes(&raw mut secret[0], 32)
    unsafe var seed : [32]u8; test_random_bytes(&raw mut seed[0], 32)
    unsafe var s_hex : [65]char; test_bytes_to_hex(&raw secret[0], 32, &raw mut s_hex[0])
    unsafe var sd_hex : [65]char; test_bytes_to_hex(&raw seed[0], 32, &raw mut sd_hex[0])
    unsafe var output : [48]u8; tls12_prf(&raw secret[0], 32, "test label" as *char, 10, &raw seed[0], 32, &raw mut output[0], 48)
    unsafe var script : [512]u8; var sp : size_t = 0; var si : size_t = 0
    ...
    unsafe var py_out_hex : [96]u8; var pw : size_t = 0; var pi : size_t = 0
...
@test public func INT_tls12_master_secret(env : &mut TestEnv) {
    unsafe var pre_master : [48]u8; test_random_bytes(&raw mut pre_master[0], 48)
    unsafe var c_random : [32]u8; test_random_bytes(&raw mut c_random[0], 32)
    unsafe var s_random : [32]u8; test_random_bytes(&raw mut s_random[0], 32)
    unsafe var ms : [48]u8; tls12_derive_master_secret(&raw pre_master[0], 48, &raw c_random[0], &raw s_random[0], &raw mut ms[0])
    unsafe var pm_hex : [97]char; test_bytes_to_hex(&raw pre_master[0], 48, &raw mut pm_hex[0])
    unsafe var cr_hex : [65]char; test_bytes_to_hex(&raw c_random[0], 32, &raw mut cr_hex[0])
    unsafe var sr_hex : [65]char; test_bytes_to_hex(&raw s_random[0], 32, &raw mut sr_hex[0])
    unsafe var script : [1024]u8; var sp : size_t = 0; var si : size_t = 0
...
@test public func INT_tls12_key_block(env : &mut TestEnv) {
    unsafe var ms : [48]u8; test_random_bytes(&raw mut ms[0], 48)
    unsafe var s_random : [32]u8; test_random_bytes(&raw mut s_random[0], 32)
    unsafe var c_random : [32]u8; test_random_bytes(&raw mut c_random[0], 32)
    unsafe var key_block : [128]u8; tls12_derive_key_block(&raw ms[0], &raw s_random[0], &raw c_random[0], &raw mut key_block[0], 128)
    unsafe var ms_hex : [97]char; test_bytes_to_hex(&raw ms[0], 48, &raw mut ms_hex[0])
    unsafe var sr_hex : [65]char; test_bytes_to_hex(&raw s_random[0], 32, &raw mut sr_hex[0])
    unsafe var cr_hex : [65]char; test_bytes_to_hex(&raw c_random[0], 32, &raw mut cr_hex[0])
    unsafe var script : [1024]u8; var sp : size_t = 0; var si : size_t = 0
    var hdr = "import hashlib,hmac\n" as *char; si=0; while(hdr[si]!=0){script[sp]=hdr[si] as u8; sp+=1; si+=1}
...
@test public func INT_mpi_exp_mod_vs_py(env : &mut TestEnv) {
    ...
    unsafe var b_hex : [64]u8; unsafe var e_hex : [64]u8; unsafe var m_hex : [64]u8; unsafe var r_hex : [64]u8
    ...
    unsafe var b : Mpi; mpi_read_binary(&raw mut b, &raw b_hex[0], 32)
    unsafe var e : Mpi; mpi_read_binary(&raw mut e, &raw e_hex[0], 32)
    unsafe var m : Mpi; mpi_read_binary(&raw mut m, &raw m_hex[0], 32)
    ...
    unsafe var chem_r : [32]u8; mpi_write_binary(&raw mut r, &raw mut chem_r[0], 32)
...
@test public func INT_mpi_mod_inv_vs_py(env : &mut TestEnv) {
    ...
    unsafe var a_hex : [32]u8; unsafe var m_hex : [32]u8
    ...
    unsafe var a : Mpi; mpi_read_binary(&raw mut a, &raw a_hex[0], 16)
    unsafe var m : Mpi; mpi_read_binary(&raw mut m, &raw m_hex[0], 16)
    unsafe var r : Mpi; mpi_init(&raw mut r)
    if(mpi_mod_inv(&raw mut r, &raw mut a, &raw mut m) < 0){env.error("mod_inv");return}else{}
    unsafe var check : Mpi; mpi_init(&raw mut check)
...
@test public func INT_mpi_gcd_vs_py(env : &mut TestEnv) {
    ...
    unsafe var a_hex : [32]u8; unsafe var b_hex : [32]u8; unsafe var g_hex : [32]u8
    test_parse_py_hex_label(&raw mut py_out, string_view("A="), &raw mut a_hex[0], 16)
    ...
    unsafe var a : Mpi; mpi_read_binary(&raw mut a, &raw a_hex[0], 16)
    unsafe var b : Mpi; mpi_read_binary(&raw mut b, &raw b_hex[0], 16)
    unsafe var g : Mpi; mpi_init(&raw mut g); mpi_gcd(&raw mut g, &raw mut a, &raw mut b)
    unsafe var chem_g : [16]u8; mpi_write_binary(&raw mut g, &raw mut chem_g[0], 16)
...
@test public func INT_mpi_shift_ops(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, 0x12345678)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_copy(&raw mut b, &raw mut a)
    mpi_shift_l(&raw mut b, 8)
    unsafe var c : Mpi; mpi_init(&raw mut c); mpi_copy(&raw mut c, &raw mut b)
...
@test public func INT_mpi_cmp_ops(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, 100)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, 200)
    unsafe var c : Mpi; mpi_init(&raw mut c); mpi_lset(&raw mut c, 100)
...
@test public func INT_tls13_record_empty(env : &mut TestEnv) {
    ...
    unsafe var enc : [32]u8; var elen = tls13_encrypt_record(&raw mut ctx, SSL_MSG_APPLICATION_DATA as u8, null, 0, &raw mut enc[0], 32)
    ...
    unsafe var dec_buf : [32]u8; var inner_ct : u8 = 0
...
@test public func INT_tls13_record_large(env : &mut TestEnv) {
    ...
    var tr_in = malloc(sizeof(Transform)) as *mut Transform; *tr_in = tr; ctx.transform_in = tr_in
    i=0; while(i<8){ctx.in_ctr[i]=0;ctx.out_ctr[i]=0;i+=1}
    unsafe var data : [16384]u8; test_random_bytes(&raw mut data[0], 16384)
    unsafe var enc : [16410]u8; var elen = tls13_encrypt_record(&raw mut ctx, SSL_MSG_APPLICATION_DATA as u8, &raw data[0], 16384, &raw mut enc[0], 16410)
    if(elen < 0){env.error("large encrypt");return}else{}
    ctx.in_hdr[0]=enc[0];ctx.in_hdr[1]=enc[1];ctx.in_hdr[2]=enc[2];ctx.in_hdr[3]=enc[3];ctx.in_hdr[4]=enc[4]
    unsafe var dec_buf : [16400]u8; var inner_ct : u8 = 0
...
@test public func INT_tls13_record_handshake_ct(env : &mut TestEnv) {
    ...
    i=0; while(i<8){ctx.in_ctr[i]=0;ctx.out_ctr[i]=0;i+=1}
    unsafe var data : [32]u8; test_random_bytes(&raw mut data[0], 32)
    unsafe var enc : [64]u8; var elen = tls13_encrypt_record(&raw mut ctx, SSL_MSG_HANDSHAKE as u8, &raw data[0], 32, &raw mut enc[0], 64)
    if(elen < 0){env.error("hs encrypt");return}else{}
    ctx.in_hdr[0]=enc[0];ctx.in_hdr[1]=enc[1];ctx.in_hdr[2]=enc[2];ctx.in_hdr[3]=enc[3];ctx.in_hdr[4]=enc[4]
    unsafe var dec_buf : [64]u8; var inner_ct : u8 = 0
...
@test public func INT_base64_roundtrip(env : &mut TestEnv) {
    unsafe var data : [48]u8; test_random_bytes(&raw mut data[0], 48)
    ...
    unsafe var dec : [48]u8; var dec_r = base64_decode(&raw enc[0], enc_len, &raw mut dec[0], 48)
...
@test public func INT_base64_vs_py(env : &mut TestEnv) {
    unsafe var data : [48]u8; test_random_bytes(&raw mut data[0], 48)
    unsafe var d_hex : [97]char; test_bytes_to_hex(&raw data[0], 48, &raw mut d_hex[0])
    unsafe var enc : [72]char; var enc_r = base64_encode(&raw data[0], 48, &raw mut enc[0], 72)
    if(enc_r is Result.Err){env.error("b64 enc");return}else{}
    var Ok(enc_len) = enc_r else unreachable
    unsafe var script : [256]u8; var sp : size_t = 0; var si : size_t = 0
    ...
    unsafe var py_enc : [72]u8; var pw : size_t = 0; var pi : size_t = 0
...
@test public func INT_compound_ecdh_gcm(env : &mut TestEnv) {
    unsafe var alice_priv : [32]u8; unsafe var alice_pub : [32]u8
    x25519_generate_keypair(&raw mut alice_priv[0], &raw mut alice_pub[0])
    unsafe var bob_priv : [32]u8; unsafe var bob_pub : [32]u8
    x25519_generate_keypair(&raw mut bob_priv[0], &raw mut bob_pub[0])
    unsafe var shared_a : [32]u8; x25519_compute_shared(&raw mut alice_priv[0], &raw bob_pub[0], &raw mut shared_a[0])
    unsafe var shared_b : [32]u8; x25519_compute_shared(&raw mut bob_priv[0], &raw alice_pub[0], &raw mut shared_b[0])
    if(!test_bytes_eq(&raw shared_a[0], &raw shared_b[0], 32)){env.error("x25519 shared mismatch");return}else{}
    unsafe var key : [16]u8; var i : size_t = 0; while(i<16){key[i]=shared_a[i]^shared_a[16+i];i+=1}
    unsafe var iv : [12]u8; test_random_bytes(&raw mut iv[0], 12)
    unsafe var pt : [40]u8; test_random_bytes(&raw mut pt[0], 40)
    unsafe var gcm : GCMContext; gcm_init(&raw mut gcm, &raw key[0], 16)
    unsafe var ct : [40]u8; unsafe var tag : [16]u8
    gcm_crypt_and_tag(&raw mut gcm, &raw iv[0], 12, null, 0, &raw pt[0], 40, &raw mut ct[0], &raw mut tag[0])
    unsafe var gcm2 : GCMContext; gcm_init(&raw mut gcm2, &raw key[0], 16)
    unsafe var dec : [40]u8
...
@test public func INT_compound_hmac_gcm(env : &mut TestEnv) {
    unsafe var key_material : [32]u8; test_random_bytes(&raw mut key_material[0], 32)
    unsafe var salt : [16]u8; test_random_bytes(&raw mut salt[0], 16)
    unsafe var hmac_key : [32]u8; hmac_sha256(&raw salt[0], 16, &raw key_material[0], 32, &raw mut hmac_key[0])
    unsafe var pt : [48]u8; test_random_bytes(&raw mut pt[0], 48)
    unsafe var iv : [12]u8; test_random_bytes(&raw mut iv[0], 12)
    unsafe var gcm : GCMContext; gcm_init(&raw mut gcm, &raw hmac_key[0], 16)
    unsafe var ct : [48]u8; unsafe var tag : [16]u8
    gcm_crypt_and_tag(&raw mut gcm, &raw iv[0], 12, null, 0, &raw pt[0], 48, &raw mut ct[0], &raw mut tag[0])
    unsafe var gcm2 : GCMContext; gcm_init(&raw mut gcm2, &raw hmac_key[0], 16)
    unsafe var dec : [48]u8
...
@test public func INT_compound_sha256_then_rsa(env : &mut TestEnv) {
    ...
    unsafe var n_hex : [256]u8
    var n_len = test_parse_py_hex_label(&raw mut py_out, string_view("N="), &raw mut n_hex[0], 256)
    if(n_len==0){env.error("rsa n");return}else{}
    unsafe var e_bytes : [3]u8; e_bytes[0]=1; e_bytes[1]=0; e_bytes[2]=1
    unsafe var msg : [32]u8; test_random_bytes(&raw mut msg[0], 32)
    unsafe var digest : [32]u8; sha256_hash(&raw msg[0], 32, &raw mut digest[0])
    unsafe var ctx : RSAContext; rsa_init(&raw mut ctx, 0, 0)
    rsa_import_pubkey(&raw mut ctx, &raw n_hex[0], n_len, &raw e_bytes[0], 3)
    unsafe var ct : [256]u8
...
@test public func INT_x509_cert_init_free(env : &mut TestEnv) {
    unsafe var cert : X509Cert; x509_cert_init(&raw mut cert)
    cert_free(&raw mut cert)
}

@test public func INT_x509_constant_time(env : &mut TestEnv) {
    unsafe var a : [16]u8; unsafe var b : [32]u8
...
@test public func INT_ecdh_p256_shared_consistency(env : &mut TestEnv) {
    unsafe var ctx_a : ECDHContext; ecdh_init(&raw mut ctx_a)
    ...
    unsafe var px : [32]u8; unsafe var py : [32]u8
...
@test public func INT_ecdh_p256_vs_py(env : &mut TestEnv) {
    ...
    unsafe var cp_hex : [65]char; test_bytes_to_hex(&raw chem_priv[0], 32, &raw mut cp_hex[0])
    unsafe var script : [1024]u8; var sp : size_t = 0; var si : size_t = 0
    ...
    unsafe var px : [32]u8; unsafe var pp : [32]u8
    test_parse_py_hex_label(&raw mut py_out, string_view("PX="), &raw mut px[0], 32)
...
@test public func INT_mpi_add_vs_py_2(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, 123456789)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, 987654321)
    unsafe var r : Mpi; mpi_init(&raw mut r); mpi_add(&raw mut r, &raw mut a, &raw mut b)
    unsafe var chem : [16]u8; mpi_write_binary(&raw mut r, &raw mut chem[0], 16)
    unsafe var script : [128]u8; var sp : size_t = 0
    var hdr = "print(format(123456789+987654321,'x'))\n" as *char; var si : size_t = 0
    while(hdr[si]!=0){script[sp]=hdr[si] as u8; sp+=1; si+=1}
    var py_out = test_python_run_script(&raw script[0], sp, string_view("mpi_add2"))
    unsafe var py_hex : [16]u8; var pw : size_t = 0; var pi : size_t = 0
...
@test public func INT_mpi_mul_vs_py_2(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, 1234567)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, 7654321)
    unsafe var r : Mpi; mpi_init(&raw mut r); mpi_mul(&raw mut r, &raw mut a, &raw mut b)
    unsafe var chem : [16]u8; mpi_write_binary(&raw mut r, &raw mut chem[0], 16)
    unsafe var script : [128]u8; var sp : size_t = 0
...
@test public func INT_mpi_div_mod(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, 1000)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, 7)
    unsafe var q : Mpi; mpi_init(&raw mut q); unsafe var r : Mpi; mpi_init(&raw mut r)
    mpi_div(&raw mut q, &raw mut r, &raw mut a, &raw mut b)
    unsafe var q_val : [8]u8; mpi_write_binary(&raw mut q, &raw mut q_val[0], 8)
    unsafe var r_val : [8]u8; mpi_write_binary(&raw mut r, &raw mut r_val[0], 8)
    if(q_val[7]!=142||r_val[7]!=6){env.error("mpi div/mod");return}else{}
}

@test public func INT_mpi_large_add(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, -1)
    mpi_shift_l(&raw mut a, 255)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, 1)
    unsafe var r : Mpi; mpi_init(&raw mut r); mpi_add(&raw mut r, &raw mut a, &raw mut b)
...
@test public func INT_consistency_p256_ng(env : &mut TestEnv) {
    unsafe var Gx : Mpi; mpi_init(&raw mut Gx); ecp_curve_gx(&raw mut Gx)
    unsafe var Gy : Mpi; mpi_init(&raw mut Gy); ecp_curve_gy(&raw mut Gy)
    unsafe var p : Mpi; mpi_init(&raw mut p); ecp_curve_p(&raw mut p)
    unsafe var b : Mpi; mpi_init(&raw mut b); ecp_curve_b(&raw mut b)
...
@test public func INT_consistency_ecdh_2g_3g(env : &mut TestEnv) {
    ...
    unsafe var k2 : Mpi; mpi_init(&raw mut k2); mpi_lset(&raw mut k2, 2)
    unsafe var R2 : ECPPoint; ecp_point_init(&raw mut R2)
    ecp_mul(&raw mut R2, &raw mut k2, &raw mut G)
    unsafe var k3 : Mpi; mpi_init(&raw mut k3); mpi_lset(&raw mut k3, 3)
    unsafe var R3 : ECPPoint; ecp_point_init(&raw mut R3)
    ecp_mul(&raw mut R3, &raw mut k3, &raw mut G)
    unsafe var P_aff : ECPPoint; ecp_point_init(&raw mut P_aff)
    ecp_curve_gx(&raw mut P_aff.X); ecp_curve_gy(&raw mut P_aff.Y); mpi_lset(&raw mut P_aff.Z, 1)
    unsafe var R2G : ECPPoint; ecp_point_init(&raw mut R2G)
...
@test public func INT_consistency_ecdh_zero_check(env : &mut TestEnv) {
    unsafe var G : ECPPoint; ecp_point_init(&raw mut G)
    ecp_curve_gx(&raw mut G.X); ecp_curve_gy(&raw mut G.Y); mpi_lset(&raw mut G.Z, 1)
    unsafe var k0 : Mpi; mpi_init(&raw mut k0); mpi_lset(&raw mut k0, 0)
...
@test public func INT_consistency_ecdh_identity(env : &mut TestEnv) {
    unsafe var G : ECPPoint; ecp_point_init(&raw mut G)
    ecp_curve_gx(&raw mut G.X); ecp_curve_gy(&raw mut G.Y); mpi_lset(&raw mut G.Z, 1)
    unsafe var k1 : Mpi; mpi_init(&raw mut k1); mpi_lset(&raw mut k1, 1)
    unsafe var R : ECPPoint; ecp_point_init(&raw mut R)
...
@test public func INT_consistency_hkdf_key_schedule(env : &mut TestEnv) {
    unsafe var ks : TLS13KeySchedule; tls13_key_schedule_init(&raw mut ks)
...
@test public func INT_gcm_null_aad_roundtrip(env : &mut TestEnv) {
    ...
    unsafe var pt : [16]u8; test_random_bytes(&raw mut pt[0], 16)
    unsafe var gcm : GCMContext; gcm_init(&raw mut gcm, &raw key[0], 16)
    unsafe var ct : [16]u8; unsafe var tag : [16]u8
...
@test public func INT_gcm_iv_variation(env : &mut TestEnv) {
    unsafe var key : [16]u8; test_random_bytes(&raw mut key[0], 16)
    unsafe var pt : [16]u8; test_random_bytes(&raw mut pt[0], 16)
    unsafe var ct1 : [16]u8; unsafe var tag1 : [16]u8
    unsafe var iv1 : [12]u8; var i : size_t = 0; while(i<12){iv1[i]=i as u8;i+=1}
    unsafe var gcm1 : GCMContext; gcm_init(&raw mut gcm1, &raw key[0], 16)
    gcm_crypt_and_tag(&raw mut gcm1, &raw iv1[0], 12, null, 0, &raw pt[0], 16, &raw mut ct1[0], &raw mut tag1[0])
    unsafe var iv2 : [12]u8; i=0; while(i<12){iv2[i]=i as u8;i+=1} iv2[0]=iv2[0]^1
    unsafe var ct2 : [16]u8; unsafe var tag2 : [16]u8
    unsafe var gcm2 : GCMContext; gcm_init(&raw mut gcm2, &raw key[0], 16)
...
@test public func INT_gcm_diff_key_diff_ct(env : &mut TestEnv) {
    unsafe var key1 : [16]u8; test_random_bytes(&raw mut key1[0], 16)
    unsafe var key2 : [16]u8; test_random_bytes(&raw mut key2[0], 16)
    unsafe var iv : [12]u8; test_random_bytes(&raw mut iv[0], 12)
    unsafe var pt : [16]u8; test_random_bytes(&raw mut pt[0], 16)
    unsafe var ct1 : [16]u8; unsafe var tag1 : [16]u8
    unsafe var gcm1 : GCMContext; gcm_init(&raw mut gcm1, &raw key1[0], 16)
    gcm_crypt_and_tag(&raw mut gcm1, &raw iv[0], 12, null, 0, &raw pt[0], 16, &raw mut ct1[0], &raw mut tag1[0])
    unsafe var ct2 : [16]u8; unsafe var tag2 : [16]u8
    unsafe var gcm2 : GCMContext; gcm_init(&raw mut gcm2, &raw key2[0], 16)
...
@test public func INT_hmac_sha256_diff_data(env : &mut TestEnv) {
    unsafe var key : [16]u8; test_random_bytes(&raw mut key[0], 16)
    unsafe var d1 : [16]u8; test_random_bytes(&raw mut d1[0], 16)
    unsafe var d2 : [16]u8; test_random_bytes(&raw mut d2[0], 16)
    unsafe var m1 : [32]u8; hmac_sha256(&raw key[0], 16, &raw d1[0], 16, &raw mut m1[0])
    unsafe var m2 : [32]u8; hmac_sha256(&raw key[0], 16, &raw d2[0], 16, &raw mut m2[0])
...
@test public func INT_hmac_sha256_diff_key(env : &mut TestEnv) {
    unsafe var k1 : [16]u8; test_random_bytes(&raw mut k1[0], 16)
    unsafe var k2 : [16]u8; test_random_bytes(&raw mut k2[0], 16)
    unsafe var data : [16]u8; test_random_bytes(&raw mut data[0], 16)
    unsafe var m1 : [32]u8; hmac_sha256(&raw k1[0], 16, &raw data[0], 16, &raw mut m1[0])
    unsafe var m2 : [32]u8; hmac_sha256(&raw k2[0], 16, &raw data[0], 16, &raw mut m2[0])
...
@test public func INT_consistency_mpi_zero(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a)
...
@test public func INT_consistency_mpi_negative(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, -10)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, 5)
    unsafe var r : Mpi; mpi_init(&raw mut r); mpi_add(&raw mut r, &raw mut a, &raw mut b)
...
@test public func INT_consistency_mpi_abs_sub(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, 100)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, 200)
    unsafe var r : Mpi; mpi_init(&raw mut r); mpi_sub(&raw mut r, &raw mut a, &raw mut b)
...
@test public func INT_consistency_mpi_mul_neg(env : &mut TestEnv) {
    unsafe var a : Mpi; mpi_init(&raw mut a); mpi_lset(&raw mut a, 7)
    unsafe var b : Mpi; mpi_init(&raw mut b); mpi_lset(&raw mut b, -3)
    unsafe var r : Mpi; mpi_init(&raw mut r); mpi_mul(&raw mut r, &raw mut a, &raw mut b)
...
@test public func INT_consistency_x25519_small_scalar(env : &mut TestEnv) {
    unsafe var scalar : [32]u8; scalar[0]=1; var i:size_t=1;while(i<32){scalar[i]=0;i+=1}
    unsafe var u_coord : [32]u8; u_coord[0]=9; i=1;while(i<32){u_coord[i]=0;i+=1}
    unsafe var out : [32]u8; x25519_ladder(&raw mut out[0], &raw scalar[0], &raw u_coord[0])
...
@test public func INT_ecdsa_p384_sha384_verify(env : &mut TestEnv) {
    ...
    unsafe var pub_key : [97]u8; pub_key[0] = 4
    var i : size_t = 0
    while(i < 48) { pub_key[1+i] = px[i]; pub_key[49+i] = py[i]; i += 1 }

    unsafe var ctx : ECDSAContext
