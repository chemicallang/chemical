@no_mangle
public func solid_symResSigNode(visitor : *mut SymResLinkSignature, node : *mut EmbeddedNode) {
...
@no_mangle
public func solid_symResNode(visitor : *mut SymResLinkBody, node : *mut EmbeddedNode) {
...
public func solid_parseMacroNode(parser : *mut Parser, builder : *mut ASTBuilder, spec : AccessSpecifier) : *mut ASTNode {
    ...
    if(parser.getToken().type == JsTokenType.LBrace as int) {
        ...
        const node = builder.make_top_level_embedded_node(spec, std::string_view("solid"), comp, node_known_type_func, node_child_res_func, cross_mod_sym_decl_proxy_fn, std::span<*mut ASTNode>(nodes_arr), std::span<*mut Value>(comp.dyn_values.data(), comp.dyn_values.size()), parser.getParentNode(), location);
