...
Value* Parser::parseAccessChain(ASTAllocator& allocator, bool parseStruct) {
    ...
    switch(tokenType) {
        ...
        case TokenType::LBrace: {
            auto ref_type = new (allocator.allocate<NamedLinkedType>()) NamedLinkedType(allocate_view(allocator, id->value), loc_single(id));
...
BaseType* Parser::ref_type_from(ASTAllocator& allocator, AccessChain* chain) {
    if(chain->values.size() == 1) {
        ...
        return new (allocator.allocate<NamedLinkedType>()) NamedLinkedType(allocate_view(allocator, val->value), val->encoded_location());
    } else {
    ...
    if (token->type == TokenType::LessThanSym && isGenericEndAhead()) {
        ...
        if(token->type == TokenType::LParen) {
            ...
            if(chain->values.size() == 1) {
                ...
                auto ref_type = new (allocator.allocate<NamedLinkedType>()) NamedLinkedType(allocate_view(allocator, id->value), id->encoded_location());
