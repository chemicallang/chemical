...
void CSTConverter::visit(FunctionParamCST *param) {
    ...
    auto isVariadic = !lastToken->compound() && lastToken->start_token()->is_abs_string() && ((AbstractStringToken*) lastToken)->value == "...";
...
void CSTConverter::visit(FunctionCST *function) {
    ...
    while(i < function->tokens.size()) {
        ...
        } else if(function->tokens[i]->start_token()->type() == LexTokenType::CharOperator && char_op(function->tokens[i].get()) == ','){
            // do nothing
        } else {
            break;
        }
    ...
    nodes.emplace_back(std::make_unique<FunctionDeclaration>(function->func_name(), std::move(params), std::move(returnType.value()), isVariadic, std::move(body)));
}
...
void CSTConverter::visit(ArrayTypeCST *arrayType) {
    ...
    if (elem_type.has_value()) {
        ...
        types.emplace_back(std::make_unique<ArrayType>(std::move(elem_type.value()), arraySize));
    } else {
        error("expected an element type for array type ", arrayType);
    }
}
