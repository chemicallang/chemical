...
- `RoutePattern`/`RouteMatch` gained a `chain : std::vector<RouteChainStep>`
  ...
  **outermost** match table as **full accumulated patterns** plus a chain
  ...
  scan on both server and client; the one behavioural difference is that a
  nested `route *` cannot catch a remainder the flattened patterns do not spell.
