...
public comptime const SEGMENTED_VECTOR_CHUNK_SIZE : size_t = 4

public struct segmented_vector_block<T> {
    var data_ptr : *mut T
    var data_size : size_t
}

public struct segmented_vector<T> {

    var blocks : *mut segmented_vector_block<T>
    ...
    func reserve_blocks(&mut self, new_cap : size_t) {
        ...
        const new_blocks = realloc(blocks, sizeof(segmented_vector_block<T>) * new_cap) as *mut segmented_vector_block<T>
        unsafe {
            if(new_blocks != null) {
                blocks = new_blocks
                block_cap = new_cap
            } else {
                panic("failed to resize segmented_vector")
            }
        }
    }
    ...
    func add_block(&mut self) {
        ...
        const data = malloc(sizeof(T) * SEGMENTED_VECTOR_CHUNK_SIZE) as *mut T
        blocks[block_count] = segmented_vector_block<T> {
            data_ptr : data,
            data_size : 0
        }
        block_count += 1
    ...
    func push(&mut self, value : T) {
        if(block_count == 0 || blocks[block_count - 1].data_size == SEGMENTED_VECTOR_CHUNK_SIZE) {
            add_block()
        }
    ...
    impl core::iterable::Chunked<T, size_t> for segmented_vector<T> {

        func begin_chunks(&self) : size_t {
            return 0
        }

        func valid_chunk(&self, c : size_t) : bool {
            return c < block_count
        }

        func current_chunk(&self, c : size_t) : core::iterable::Chunk<T> {
            return core::iterable::Chunk<T> {
                ptr : blocks[c].data_ptr,
                len : blocks[c].data_size
            }
        }

        func next_chunk(&self, c : size_t) : size_t {
            return c + 1
        }
        ...
        func previous_chunk(&self, c : size_t) : size_t {
            return c - 1
        }

        func total_size(&self) : size_t {
            return data_size
        }
