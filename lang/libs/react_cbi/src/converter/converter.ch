...
func (converter : &mut JsConverter) convertJSXComponent(element : *mut JsJSXElement, tagName : std::string_view, tagNameNode : *mut JsNode) {
    ...
    if(element.componentSignature != null && element.componentSignature.mountStrategy == MountStrategy.Universal) {
        ...
        converter.str.append_view("return $r_uni_ch(html, ");
 ...
 func (converter : &mut JsConverter) convertJSXNativeElement(element : *mut JsJSXElement, tagName : std::string_view) {
    ...
    // Helper to check if we have multiple classes to merge
            ...
            if(isClass) {
                ...
                if(classAttrs.size() > 1) {
                    ...
                    for(var j : uint = 0; j < classAttrs.size(); j++) {
                        ...
                        converter.convertAttributeValue(classAttrs.get(j));
                    }
                    converter.str.append_view("].join(' ')");
                } else {
                    converter.convertAttributeValue(classAttrs.get(0));
                }
                classMerged = true;
                attrCount++;
            } else {
