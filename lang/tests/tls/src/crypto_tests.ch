...
@test public func TEST_mpi_mul_int_vs_py(env:&mut TestEnv) {
    ...
    unsafe var p_bytes : [32]u8; mpi_get_bytes(&raw mut p, &raw mut p_bytes[0])
    unsafe var p_hex : [65]char; test_bytes_to_hex(&raw p_bytes[0], 32, &raw mut p_hex[0])
    ...
    unsafe var py_r:[64]u8; var py_len : size_t = 0
    var pi : size_t = 0
    ...
    var cs = mpi_size(&raw mut mr); unsafe var cb:[64]u8; mpi_write_binary(&raw mut mr, &raw mut cb[0], 64)
...
@test public func TEST_mpi_sub_vs_py(env:&mut TestEnv) {
    ...
    unsafe var py_r:[40]u8; var py_len : size_t = 0
    var pi : size_t = 0
    ...
    unsafe var mf: Mpi; unsafe var fv:[1]u8; fv[0]=5; mpi_read_binary(&raw mut mf, &raw fv[0], 1)
    unsafe var mr: Mpi; mpi_init(&raw mut mr)
    var ret = mpi_sub(&raw mut mr, &raw mut p, &raw mut mf)
    var cs = mpi_size(&raw mut mr); unsafe var cb:[40]u8; mpi_write_binary(&raw mut mr, &raw mut cb[0], 40)
...
@test public func TEST_mpi_mul_vs_py(env:&mut TestEnv) {
    ...
    unsafe var mtwo: Mpi; unsafe var tv:[1]u8; tv[0]=2; mpi_read_binary(&raw mut mtwo, &raw tv[0], 1)
    unsafe var mr: Mpi; mpi_init(&raw mut mr)
...
@test public func TEST_mpi_mod_vs_py(env:&mut TestEnv) {
    ...
    unsafe var py_r:[8]u8; var py_len : size_t = 0
    var pi : size_t = 0
    ...
    unsafe var mf: Mpi; unsafe var fv:[1]u8; fv[0]=5; mpi_read_binary(&raw mut mf, &raw fv[0], 1)
    unsafe var mm: Mpi; unsafe var mv:[1]u8; mv[0]=7; mpi_read_binary(&raw mut mm, &raw mv[0], 1)
    unsafe var tmp: Mpi; mpi_init(&raw mut tmp); mpi_add(&raw mut tmp, &raw mut p, &raw mut mf)
    unsafe var mr: Mpi; mpi_init(&raw mut mr); mpi_mod(&raw mut mr, &raw mut tmp, &raw mut mm)
    var cs = mpi_size(&raw mut mr); unsafe var cb:[8]u8; mpi_write_binary(&raw mut mr, &raw mut cb[0], 8)
...
@test public func TEST_mpi_negmod_vs_py(env:&mut TestEnv) {
    ...
    unsafe var mthree: Mpi; unsafe var tv:[1]u8; tv[0]=3; mpi_read_binary(&raw mut mthree, &raw tv[0], 1)
    unsafe var tmp: Mpi; mpi_init(&raw mut tmp); var ret = mpi_sub(&raw mut tmp, &raw mut mthree, &raw mut p)
    if(ret < 0) { env.error("mpi_sub failed"); return }
    unsafe var mr: Mpi; mpi_init(&raw mut mr); ret = mpi_mod(&raw mut mr, &raw mut tmp, &raw mut p)
...
@test public func TEST_p256_prime(env:&mut TestEnv) {
    unsafe var p : Mpi; ecp_curve_p(&raw mut p)
    unsafe var expected : Mpi; ecp_curve_p(&raw mut expected)
    unsafe var buf1:[32]u8; unsafe var buf2:[32]u8
...
@test public func TEST_ecdh_k2_direct(env:&mut TestEnv) {
    ...
    unsafe var chem_x : [32]u8; ret = mpi_write_binary(&raw mut R.X, &raw mut chem_x[0], 32)
    if(ret < 0) { snprintf(&raw mut _k2b[0], sizeof(_k2b), "[K2] write error=%d", ret); env.info(&raw _k2b[0]); env.error("write failed"); return }

    // Python: compute 2*G
    unsafe var py_script : [512]u8; var psi:size_t=0
    ...
    unsafe var py_pub:[65]u8; var py_len = run_py_script(&raw py_script[0], psi, &raw mut py_pub[0], 65)
...
@test public func TEST_ecdh_add(env:&mut TestEnv) {
    ...
    unsafe var R3_add : ECPPoint; ecp_point_init(&raw mut R3_add)
    ...
    unsafe var add_x : [32]u8; mpi_write_binary(&raw mut R3_add.X, &raw mut add_x[0], 32)

    unsafe var k3 : Mpi; mpi_init(&raw mut k3); mpi_lset(&raw mut k3, 3)
    unsafe var R3_mul : ECPPoint; ecp_point_init(&raw mut R3_mul)
    ...
    unsafe var mul_x : [32]u8; mpi_write_binary(&raw mut R3_mul.X, &raw mut mul_x[0], 32)
...
@test public func TEST_ecdh_k3_direct(env:&mut TestEnv) {
    ...
    unsafe var G : ECPPoint; ecp_point_init(&raw mut G)
    ecp_curve_gx(&raw mut G.X); ecp_curve_gy(&raw mut G.Y); mpi_lset(&raw mut G.Z, 1)
    unsafe var k : Mpi; mpi_init(&raw mut k); mpi_lset(&raw mut k, 3)
    unsafe var R : ECPPoint; ecp_point_init(&raw mut R)
    var ret = ecp_mul(&raw mut R, &raw mut k, &raw mut G)
...
@test public func TEST_curve_equation_stepwise(env:&mut TestEnv) {
    ...
    ecp_curve_gx(&raw mut G.X); ecp_curve_gy(&raw mut G.Y); mpi_lset(&raw mut G.Z, 1)
    unsafe var p : Mpi; ecp_curve_p(&raw mut p)
    unsafe var b_m : Mpi; ecp_curve_b(&raw mut b_m)
...
@test public func TEST_mpi_mul_simple(env:&mut TestEnv) {
    ...
    unsafe var ma:Mpi; mpi_read_binary(&raw mut ma, &raw a[0], 32)
    unsafe var mr:Mpi; mpi_init(&raw mut mr)
...
@test public func TEST_sha512_incremental_matches(env:&mut TestEnv) {
    unsafe var msg : [1000]u8
    var i : usize = 0
    while(i < 1000) { msg[i] = (i % 251) as u8; i += 1 }

    unsafe var one : [64]u8
    crypto::sha512_hash(&raw msg[0], 1000, &raw mut one[0])

    unsafe var inc : [64]u8
    unsafe var ctx : crypto::Sha512Context
