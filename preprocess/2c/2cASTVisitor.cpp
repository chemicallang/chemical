...
/**
 * evaluates the given comptime function call and returns its evaluated value.
 * the returned value of a %runtime_value / %runtime_block_value is fully
 * self-contained: when it was returned (set_return), evaluated_value created
 * an evaluated copy of the runtime value (evaluating the comptime variables
 * captured from the function's scope into the copy's refs), so it can be
 * translated by the visitor after the interpret scope has been destroyed
 */
...
void write_lambda_expansion_site(ToCAstVisitor& visitor, LambdaFunction* lambdaFn, BaseType* type, CapturingFunctionType* cap_type, bool is_reference) {
            ...
            if (imp_cons->is_comptime()) {
                ...
                for (const auto captured: lambdaFn->captureList) {
                    ...
                    const auto identifier = new(
                            visitor.allocator.allocate<VariableIdentifier>()) VariableIdentifier(
                            captured->name, captured->encoded_location(), false
                    );
                    identifier->linked = captured->linked;
                    set_moved_ref_drop_flag(visitor, identifier);
                    visitor.space();
                }

                visitor.write(visitor.fat_pointer_type);
                visitor.write("* ");
                visitor.write_str(lambda_name);
                visitor.write("_pair");
                visitor.write(" = ");
                if (lambdaFn->captureList.empty()) {
                    visitor.write("&(__chemical_fat_pointer__){");
                    write_lambda_call_site(visitor, lambdaFn, lambda_name);
                    visitor.write(",NULL}");
                } else {
                    write_lambda_call_site(visitor, lambdaFn, lambda_name);
                }
                ...
                // evaluated result is written directly into the output buffer
                // (inside the compound expression)
                const auto eval = evaluate_comptime_func(visitor, imp_cons, imp_cons_call);
                if(eval) {
                ...
                call_implicit_constructor(visitor, imp_cons, lambdaFn, false);
            }
        }
    }
}
    ...
    if(type) {
        ...
        if (value->kind() == ValueKind::ReferenceOfValue) {
            ...
            if (maybe_lamb->kind() == ValueKind::LambdaFunc) {
                ...
                const auto cap_type = cap_type_canonical->get_cap_func_type();
                if (cap_type != nullptr) {
                    write_lambda_expansion_site(*this, maybe_lamb->as_lambda_func_unsafe(), type, cap_type, true);
                    return;
                }
            }
    ...
    if(type) {
        ...
        if (imp_cons) {
            if(imp_cons->is_comptime()) {
                ...
                const auto eval = evaluate_comptime_func(*this, imp_cons, imp_cons_call);
                if(eval) {
...
void call_single_arg_operator_func(ToCAstVisitor& visitor, FunctionDeclaration* func, Value* value) {
    if(func->params.size() != 1) {
        visitor.write('0');
        visitor.error(value) << "operator implementation with name '" << func->name_view() << "' must have exactly a single parameter";
        return;
    }
...
void assign_statement(ToCAstVisitor& visitor, AssignStatement* assign) {
    ...
    if(assign->assOp != Operation::Assignment) {
        ...
        if(can_node) {
            ...
            if(container) {
                const auto impl = visitor.implsIndex.get_ass_op_impl(visitor.coreNodes, container, assign->assOp);
                if (impl == nullptr) {
                    visitor.write('0');
                    visitor.error("couldn't overload this operator", assign);
                    return;
                }
                call_two_arg_operator_func(visitor, impl, assign->lhs, assign->value);
                return;
            }
        }
    }
...
void ToCAstVisitor::VisitIncDecValue(IncDecValue *value) {
    ...
    if(node) {
        ...
        if(container) {
            const auto func = implsIndex.get_inc_dec_op_impl(coreNodes, container, value->increment, value->post);
            if (func == nullptr) {
                error(value) << "couldn't find overloaded operator";
                return;
            }
            call_single_arg_operator_func(*this, func, value->getValue());
            return;
        }
    }
...
struct ArgDestructionDep {
    ...
    Value* arg_val;
    // set when the variable was already allocated by an outer scope,
    // so the inner scope should skip destructing it
...
FunctionCall* get_last_call(Value* value) {
    switch (value->kind()) {
        ...
        case ValueKind::AddrOfValue:
            return get_last_call(value->as_addr_of_value_unsafe()->value);
        case ValueKind::ReferenceOfValue:
            return get_last_call(value->as_reference_of_value_unsafe()->value);
        default:
            return nullptr;
    }
}
...
void dep_for_destr_obj_to_ref(ArgsDestructionInfo& info, Value* val, BaseType* type) {
    ...
    if (last_call) {
    ...
    } else if (val->kind() == ValueKind::StructValue || (val->kind() == ValueKind::AddrOfValue && val->as_addr_of_value_unsafe()->value->kind() == ValueKind::StructValue)) {
        ...
        if (def) {
            const auto destrFunc = def->destructor_func();
            if (destrFunc) {
                auto actual_val = val->kind() == ValueKind::AddrOfValue ? val->as_addr_of_value_unsafe()->value : val;
                info.deps.emplace_back(ArgDestructionDep {
                    .decl = def,
                    .destr = destrFunc,
                    .arg_val = actual_val
...
void dep_for_temporary_struct_chain(ArgsDestructionInfo& info, Value* val) {
    AccessChain* chain = nullptr;
    if(val->kind() == ValueKind::AccessChain) {
        chain = val->as_access_chain_unsafe();
    } else if(val->kind() == ValueKind::FunctionCall) {
        const auto call = val->as_func_call_unsafe();
        const auto parent = call->parent_val;
        if(parent && parent->kind() == ValueKind::AccessChain) {
            chain = parent->as_access_chain_unsafe();
        }
    }
    if(!chain) return;
    if(chain->values.size() > 1) {
        ...
        const auto first = chain->values.front();
        const auto innerCall = get_only_call(first);
        if(innerCall) {
            const auto type = innerCall->getType();
            const auto container = type->get_direct_linked_container();
...
void ToCAstVisitor::VisitFunctionCall(FunctionCall *call) {
    ...
    if(func_decl) {
        if(func_decl->is_comptime()) {
            // a comptime call captured inside a %runtime_value /
            // %runtime_block_value was already evaluated while the enclosing
            // comptime scope was alive (its result is stored in the runtime
            // value's captured refs, which the call node points to). its stored
            // result is translated here. this matters for location-aware
            // intrinsics and for arguments that reference the enclosing
            // comptime function's locals, which cannot be resolved after the
            // scope has been destroyed
            if(call->captured_ref_owner && call->captured_ref_index >= 0 && call->captured_ref_index < (int) call->captured_ref_owner->refs.size()) {
                const auto captured = call->captured_ref_owner->refs[call->captured_ref_index].evaluated;
                if(captured) {
                    visit(captured);
                    return;
                }
            }
            // the returned value is fully self-contained (a returned
            // %runtime_value / %runtime_block_value evaluated its captured
            // comptime variables into an evaluated copy at return time), so it
            // is translated right here. functions that return the value
            // directly without going through set_return (intrinsics like
            // intrinsics::size that override call) are handled the same way
            const auto eval = evaluate_comptime_func(*this, func_decl, call);
            if(eval) {
                visit(eval);
            }
            return;
    ...
    if(ASTNode::isAnyStructMember(linked_kind)) {
        ...
        switch(linked_kind) {
            ...
            case ASTNodeKind::CapturedComptimeVariable: {
                // variable captured from the comptime environment. the returned
                // %runtime_value / %runtime_block_value evaluated the captured
                // variables when it was returned (evaluated_value created an
                // evaluated copy), so the value is resolved through the owning
                // runtime value's refs (parent + index)
                const auto evl = linked->as_captured_comptime_var_unsafe();
                if(!evl->parent_refs || evl->index >= evl->parent_refs->refs.size() || !evl->parent_refs->refs[evl->index].evaluated) {
                    error("couldn't find the value of the captured comptime variable", evl);
                    write("missing_captured_comptime_variable_value");
                    return;
                }
                const auto val = evl->parent_refs->refs[evl->index].evaluated;
...
void overload_expr_operator(ToCAstVisitor& visitor, MembersContainer* container, Expression* expr) {
    const auto overloaded = visitor.implsIndex.get_expr_op_impl(visitor.coreNodes, container, expr->operation);
    if (overloaded == nullptr) {
        visitor.write('0');
        visitor.error("couldn't find operator function", expr);
        return;
    }
...
void ToCAstVisitor::VisitNegativeValue(NegativeValue *negValue) {
    ...
    if(node) {
        ...
        if(container) {
            ...
            if (func == nullptr) {
                error(negValue) << "couldn't get overloaded operator implementation function";
                return;
            }
...
void ToCAstVisitor::VisitNotValue(NotValue *notValue) {
    ...
    if(node) {
        ...
        if(container) {
            ...
            if (func == nullptr) {
                error(notValue) << "couldn't get overloaded operator implementation function";
                return;
            }
            call_single_arg_operator_func(*this, func, val);
            return;
