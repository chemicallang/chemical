...
func write_tls_python_utils() {
    ...
    py.append_view("    if t=='ec384':\n")
    py.append_view("        key=ec.generate_private_key(ec.SECP384R1())\n")
    py.append_view("    elif t=='ec':\n")
    py.append_view("        key=ec.generate_private_key(ec.SECP256R1())\n")
    py.append_view("    else:\n")
    py.append_view("        key=rsa.generate_private_key(65537,2048)\n")
    ...
    py.append_view("        t=ctx.wrap_socket(s,server_hostname=host)\n")
     ...
     py.append_view("        open('/tmp/tls_cli_err.txt','w').write('WRAP_ERROR:'+repr(e))\n")
     py.append_view("        s.close()\n")
    ...
    py.append_view("    except Exception:\n")
    py.append_view("        pass\n")
    py.append_view("def tls_cli_frag(host,port,ver='1.3',ciph=None,frag=16):\n")
    py.append_view("    ctx=ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)\n")
    py.append_view("    ctx.check_hostname=False\n")
    py.append_view("    ctx.verify_mode=ssl.CERT_NONE\n")
    py.append_view("    ctx.minimum_version=ssl.TLSVersion.TLSv1_3 if ver=='1.3' else ssl.TLSVersion.TLSv1_2\n")
    py.append_view("    if ver!='1.3':ctx.maximum_version=ssl.TLSVersion.TLSv1_2\n")
    py.append_view("    if ciph:ctx.set_ciphers(ciph)\n")
    py.append_view("    class FragSocket(socket.socket):\n")
    py.append_view("        def __init__(self,*a,**k):\n")
    py.append_view("            super().__init__(*a,**k)\n")
    py.append_view("            self._raw_send=super().send\n")
    py.append_view("            self._frag=frag\n")
    py.append_view("        def send(self,b,*a,**k):\n")
    py.append_view("            step=self._frag\n")
    py.append_view("            i=0;n=0\n")
    py.append_view("            while i<len(b):\n")
    py.append_view("                m=self._raw_send(b[i:i+step],*a,**k)\n")
    py.append_view("                if m is None or m<=0:\n")
    py.append_view("                    return m if n==0 else n\n")
    py.append_view("                n+=m;i+=m\n")
    py.append_view("            return n\n")
    py.append_view("    s=FragSocket(socket.AF_INET,socket.SOCK_STREAM)\n")
    py.append_view("    s.settimeout(10)\n")
    py.append_view("    s.connect((host,int(port)))\n")
    py.append_view("    try:\n")
    py.append_view("        t=ctx.wrap_socket(s,server_hostname=host)\n")
    py.append_view("        t.sendall(b'GET / HTTP/1.0\\r\\n\\r\\n')\n")
    py.append_view("        d=t.recv(4096)\n")
    ...
    py.append_view("            pass\n")
    py.append_view("    s.close()\n")
    py.append_view("def rawcli(host,port,payload):\n")
    py.append_view("    import time\n")
    py.append_view("    try:\n")
    py.append_view("        s=socket.create_connection((host,int(port)),timeout=10)\n")
    py.append_view("        s.sendall(payload.encode() if isinstance(payload,str) else payload)\n")
    py.append_view("        time.sleep(3)\n")
    py.append_view("        s.close()\n")
    py.append_view("    except Exception:\n")
    py.append_view("        pass\n")
    ...
    py.append_view("            cn.close()\n")
    py.append_view("        elif mode=='methods':\n")
    py.append_view("            seq=[('GET',None),('POST',b'POSTDATA'),('PUT',b'PUTDATA'),('DELETE',None),('PATCH',b'P1')]\n")
    py.append_view("            exp={'GET':b'M:GET','POST':b'POSTDATA','PUT':b'PUTDATA','DELETE':b'M:DELETE','PATCH':b'P1'}\n")
    py.append_view("            ok=True;why=''\n")
    py.append_view("            for mtd,bd in seq:\n")
    py.append_view("                s=None\n")
    py.append_view("                for att in range(50):\n")
    py.append_view("                    try:\n")
    py.append_view("                        s=socket.create_connection((host,int(port)),timeout=20)\n")
    py.append_view("                        break\n")
    py.append_view("                    except (ConnectionRefusedError,OSError):\n")
    py.append_view("                        time.sleep(0.1)\n")
    py.append_view("                if s is None:\n")
    py.append_view("                    ok=False;why+=' %s:noroute'%mtd\n")
    py.append_view("                    continue\n")
    py.append_view("                try:\n")
    py.append_view("                    ts=cc.wrap_socket(s,server_hostname='localhost')\n")
    py.append_view("                    cn=http.client.HTTPConnection(host,int(port),timeout=20)\n")
    py.append_view("                    cn.sock=ts\n")
    py.append_view("                    cn.request(mtd,'/m',body=bd)\n")
    py.append_view("                    r=cn.getresponse();d=r.read();cn.close()\n")
    py.append_view("                    if r.status!=200 or d!=exp[mtd]:\n")
    py.append_view("                        ok=False;why+=' %s:%s/%r'%(mtd,r.status,d[:30])\n")
    py.append_view("                except Exception as ex:\n")
    py.append_view("                    ok=False;why+=' %s:EX:%s'%(mtd,str(ex)[:60])\n")
    py.append_view("                time.sleep(0.05)\n")
    ...
    py.append_view("def rawsrv(cert,key,port,nconn=3):\n")
    py.append_view("    import os\n")
    py.append_view("    ctx=ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)\n")
    py.append_view("    ctx.load_cert_chain(cert,key)\n")
    py.append_view("    s=socket.socket()\n")
    ...
    py.append_view("    s.listen(int(nconn))\n")
    py.append_view("    s.settimeout(30)\n")
    py.append_view("    resps=[b'HTTP/1.1 200 \\r\\nContent-Length: 2\\r\\nConnection: close\\r\\n\\r\\nOK',b'HTTP/1.1 201 made\\r\\ncontent-length:2\\r\\nx-lower:yes\\r\\nconnection:close\\r\\n\\r\\nhi',b'HTTP/1.0 404 lost\\r\\nContent-Length: 4\\r\\nX-Multi: a\\r\\nX-Multi: b\\r\\nConnection: close\\r\\n\\r\\nnope']\n")
    py.append_view("    for i in range(int(nconn)):\n")
    py.append_view("        try:\n")
    py.append_view("            c,a=s.accept()\n")
    py.append_view("            t=ctx.wrap_socket(c,server_side=True)\n")
    py.append_view("            d=t.recv(4096)\n")
    py.append_view("            t.sendall(resps[i])\n")
    py.append_view("            t.close()\n")
    py.append_view("        except Exception as e:\n")
    py.append_view("            print('RAWSRV_ERR:'+str(e),file=sys.stderr)\n")
    py.append_view("    s.close()\n")
    py.append_view("def hangsrv(cert,key,port):\n")
    py.append_view("    import os\n")
    py.append_view("    ctx=ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)\n")
    py.append_view("    ctx.load_cert_chain(cert,key)\n")
    py.append_view("    s=socket.socket()\n")
    ...
    py.append_view("        nbytes=(key.curve.key_size+7)//8; val=key.private_numbers().private_value; open(sys.argv[3],'w').write(format(val,'0{}x'.format(nbytes*2)))\n")
    py.append_view("    else:\n")
    ...
    py.append_view("elif cmd=='clifrag':\n")
    py.append_view("    tls_cli_frag(sys.argv[2],sys.argv[3],sys.argv[4] if len(sys.argv)>4 else '1.3',sys.argv[5] if len(sys.argv)>5 else None,int(sys.argv[6]) if len(sys.argv)>6 else 16)\n")
    ...
    py.append_view("elif cmd=='rawcli':\n")
    py.append_view("    rawcli(sys.argv[2],sys.argv[3],sys.argv[4] if len(sys.argv)>4 else 'GET')\n")
    ...
    py.append_view("elif cmd=='rawsrv':\n")
