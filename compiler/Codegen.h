...
class Codegen : public ASTDiagnoser, public ComptimeReturnHandler {
public:
    ...
    /**
     * llvm values generated by the return handler, keyed by the comptime
     * function call. the handler generates the returned value's llvm ir while
     * the comptime interpret scope is still alive, and FunctionCall::llvm_value
     * retrieves it from here instead of generating it again (which would
     * require the now dead interpret scope)
     */
    std::unordered_map<FunctionCall*, llvm::Value*> comptime_return_handled_values;

    /**
     * implementation of ComptimeReturnHandler, generates llvm ir for the
     * returned runtime value while the interpret scope is still alive
     */
    void handle_return_value(InterpretScope& scope, Value* value) override;
