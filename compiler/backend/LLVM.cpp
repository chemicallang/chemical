...
llvm::Type *IntNType::llvm_type(Codegen &gen) {
    const auto ty = gen.builder->getIntNTy(num_bits(gen.comptime_scope.target_data));
#ifdef DEBUG
    if(!ty) {
...
llvm::Type *PointerType::llvm_chain_type(Codegen &gen, std::vector<Value*> &values, unsigned int index) {
...
llvm::Type *ReferenceType::llvm_chain_type(Codegen &gen, std::vector<Value*> &values, unsigned int index) {
    if(index == values.size() - 1) {
        return gen.builder->getPtrTy();
    } else {
        return type->llvm_chain_type(gen, values, index);
...
llvm::Type *LinkedType::llvm_chain_type(Codegen &gen, std::vector<Value*> &values, unsigned int index) {
...
llvm::Type *StructType::llvm_chain_type(Codegen &gen, std::vector<Value*> &values, unsigned int index) {
...
llvm::Type *UnionType::llvm_chain_type(Codegen &gen, std::vector<Value*> &values, unsigned int index) {
...
llvm::Type *IntNumValue::llvm_type(Codegen &gen) {
    return gen.builder->getIntNTy(get_num_bits(gen.comptime_scope.target_data));
}

llvm::Value *IntNumValue::llvm_value(Codegen &gen, BaseType* expected_type) {
    return gen.builder->getIntN(get_num_bits(gen.comptime_scope.target_data), get_num_value());
}
...
llvm::Type *VariableIdentifier::llvm_chain_type(Codegen &gen, std::vector<Value*> &chain, unsigned int index) {
...
llvm::Value *CapturedComptimeVariable::llvm_pointer(Codegen &gen) {
    ...
    // when the captured variable is itself a pointer/reference, its value is
    // already an address. Resolve the value (load it), instead of producing the
    // address of the storage holding it: a captured pointer local passed to a
    // `%runtime_value` expression (e.g. `builder.allocate<T>()` where
    // `var builder = md.builder`) must pass `builder`, not `&builder`.
    const auto captured_type = known_type();
    if(captured_type != nullptr) {
        const auto captured_kind = captured_type->kind();
        if(captured_kind == BaseTypeKind::Pointer || captured_kind == BaseTypeKind::Reference) {
...
llvm::Value *Expression::llvm_value(Codegen &gen, BaseType* expected_type) {
    ...
    replace_number_values(gen.allocator, gen.comptime_scope.typeBuilder, first_pure, second_pure, operation, gen.comptime_scope.target_data);
...
llvm::Value *CastedValue::llvm_value(Codegen &gen, BaseType* expected_type) {
    ...
    if(value_type->kind() == BaseTypeKind::IntN && pure_type->kind() == BaseTypeKind::IntN) {
        ...
        const auto from_num_bits = from_num_type->num_bits(gen.comptime_scope.target_data);
        const auto to_num_bits = to_num_type->num_bits(gen.comptime_scope.target_data);
...
llvm::Value* IsValue::llvm_value(Codegen &gen, BaseType* expected_type) {
    ...
    if(comp_time.has_value()) {
        ...
        if(linked != nullptr && linked->kind() == ASTNodeKind::VariantMember) {
            ...
            llvm::Value* idxList[] = { gen.builder->getInt32(0), gen.builder->getInt32(0) };
            const auto gep = gen.builder->CreateGEP(def_type, expr_value, llvm::ArrayRef<llvm::Value*>(idxList), "",gen.inbounds);
...
// A replacement `Scope` returned by a CBI macro (e.g. the impls generated by
// `#json`) is handed to EmbeddedNode::code_gen_declare. Recurse into it so the
// contained declarations (impls, functions, ...) get their declare pass run.
// Without this, macro-generated impls get bodies but never declare, so the
// trait_impl_func_map stays empty and body generation fails with "failed to
// find declared function pointer when implementing body".
void Scope::code_gen_declare(Codegen &gen) {
    gen_declare_top_level(gen);
}

void Scope::code_gen_external_declare(Codegen &gen) {
    external_declare_top_level(gen);
}
