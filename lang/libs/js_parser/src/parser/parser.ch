...
func (jsParser : &mut JsParser) parseArrayDestructuring(parser : *mut Parser, builder : *mut ASTBuilder) : *mut JsNode {
    ...
    while(true) {
        ...
        if(parser.getToken().type == JsTokenType.RBracket as int) break;
...
func (jsParser : &mut JsParser) parseImport(parser : *mut Parser, builder : *mut ASTBuilder) : *mut JsNode {
    ...
    if(parser.getToken().type == JsTokenType.Identifier as int) {
        ...
        specifiers.push(ImportSpecifier { imported : std::string_view("default"), local : local });
        parser.increment();
        if(parser.getToken().type == JsTokenType.Comma as int) {
            parser.increment();
        }
    ...
    if(parser.getToken().type == JsTokenType.Star as int) {
        ...
        parser.increment(); // *
        if(parser.getToken().type == JsTokenType.Identifier as int && parser.getToken().value.equals(std::string_view("as"))) {
            parser.increment(); // as
            if(parser.getToken().type == JsTokenType.Identifier as int) {
                 var local = builder.allocate_view(&parser.getToken().value);
                 specifiers.push(ImportSpecifier { imported : std::string_view("*"), local : local });
                 parser.increment();
            } else {
    ...
    // Expect 'from'
    if(parser.getToken().type == JsTokenType.Identifier as int && parser.getToken().value.equals(std::string_view("from"))) {
        parser.increment();
    } else {
