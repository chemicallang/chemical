...
@no_mangle
public func preact_replacementNodeDeclare(builder : *mut ASTBuilder, value : *mut EmbeddedNode) : *ASTNode {
...
@no_mangle
public func preact_replacementNode(builder : *mut ASTBuilder, value : *mut EmbeddedNode) : *ASTNode {
    const root = value.getDataPtr() as *mut JsComponentDecl;
...
public func preact_parseMacroNode(parser : *mut Parser, builder : *mut ASTBuilder, spec : AccessSpecifier) : *mut ASTNode {
    ...
    if(parser.getToken().type == JsTokenType.LBrace as int) {
        ...
        const node = builder.make_embedded_node(std::string_view("preact"), comp, node_known_type_func, node_child_res_func, std::span<*mut ASTNode>(nodes_arr), std::span<*mut Value>(comp.dyn_values.data(), comp.dyn_values.size()), parser.getParentNode(), location);
...
@no_mangle
public func preact_initializeLexer(lexer : *mut Lexer) {
