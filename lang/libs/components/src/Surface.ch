...
public #universal Dialog(props) {
    ...
    // Expose the active dialog's dismiss handler so the built-in close button in
    // DialogContent (which is not passed onClose directly) can close the dialog.
    useEffect(() => {
        if(isOpen) {
            window.$__uni_dialog_close = close
            return () => { if(window.$__uni_dialog_close === close) { window.$__uni_dialog_close = null } }
        }
        return () => {}
    }, [isOpen])
