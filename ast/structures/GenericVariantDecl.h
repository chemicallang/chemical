 ...
 */
class GenericVariantDecl : public GenericMembersDecl {
public:

    /**
     * finalize the signature of the struct decl, it means copy the signature in place
     */
    static void finalize_signature(ASTAllocator& allocator, VariantDefinition* def);

    /**
     * finalize the body of the struct decl, it means copy the body in place
     */
    static void finalize_body(ASTAllocator& allocator, VariantDefinition* def);

    /**
     * master implementation is the first implementation we encounter
     */
    VariantDefinition* master_impl;
     ...
     */
    std::vector<VariantDefinition*> instantiations;
    ...
    ) : GenericMembersDecl(ASTNodeKind::GenericVariantDecl, parent_node, location), master_impl(master_impl) {
     ...
     */
    VariantDefinition* instantiate_type(GenericInstantiatorAPI& instantiator, std::vector<TypeLoc>& types, SourceLocation location);
