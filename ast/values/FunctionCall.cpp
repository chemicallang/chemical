...
bool variant_call_initialize(Codegen &gen, llvm::Value* allocated, llvm::Type* def_type, VariantMember* member, FunctionCall* call) {
    ...
    for(auto& value : member->values) {
        const auto param = value.second;

        // determine the value
        Value* value_ptr;
        if(i < total_args) {
            value_ptr = call->values[i];
        } else {
            if(param->def_value) {
                value_ptr = param->def_value;
            } else {
                gen.error(call) << "no argument given for argument " << std::to_string(i) << ", no default value exists";;
                return false;
            }
        }
        ...
        // store in struct
        value_ptr->store_in_struct(gen, call, data_ptr, struct_type, {gen.builder->getInt32(0)}, i, param_type);
...
Value* FunctionCall::evaluated_value(InterpretScope &scope) {
    ...
    if(parent && parent->val_kind() == ValueKind::FunctionCall && evaluated_parent && evaluated_parent != result) {
        scope.destroy_value(evaluated_parent);
    }
    return result;
}
