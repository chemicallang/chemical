...
| 4 — control API, modes, hooks | **Partial** | `router(...)` lowers to `window.$__uni_router(...)` (D-3.1), `preload`/`remote`/`noscroll` modes, public `Link`/`NavLink`/`Outlet` components, hooks (`onActivate`/`onDeactivate`/`onBeforeActivate`) converted into the registration stub, `route title` + 404 `noindex`, and diagnostics **R8** (literal `activateRoute`/`preload`/`buildPath`/`replaceRoute` ids) and **R14** (`$__uni_*` in route bodies/hooks) via a JS-AST walk. **Not yet:** R11/R12 (they read `props.X` inside the route *component*, which is a different component from the router declaration). |
