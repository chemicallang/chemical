...
func neg_for_in_non_iterable_full(env : &mut TestEnv) {
    ...
    var ch = "func main() {\n    for(var x in 42) { }\n}\n"
    expect_compile_error(env, "for_in_non_iterable", ch, "couldn't get container")
}
...
func neg_for_in_non_container(env : &mut TestEnv) {
    mkdir(NEG_WORK_DIR, 0o777 as uint)
    var ch = "func main() {\n    for(var x in \"hello\") { }\n}\n"
    expect_compile_error(env, "for_in_string", ch, "couldn't get container")
}
...
func neg_for_in_unknown_size(env : &mut TestEnv) {
    mkdir(NEG_WORK_DIR, 0o777 as uint)
    var ch = "func main() {\n    var p : *int = null\n    for(var x in p) { }\n}\n"
    expect_compile_error(env, "for_in_ptr", ch, "couldn't get container")
}
...
func neg_reverse_iter_no_rev(env : &mut TestEnv) {
    ...
    // 'core::iterable::Iterable' reference does not resolve; the code is
    // rejected with an unresolved identifier error instead.
    expect_compile_error(env, "reverse_iter_no_rev", ch, "unresolved")
}
...
func neg_iter_current_not_ref(env : &mut TestEnv) {
    ...
    // NOTE: the test module cannot import the 'core' library, so the
    // 'core::iterable::Iterable' reference does not resolve.
    expect_compile_error(env, "iter_current_not_ref", ch, "unresolved")
}
...
func neg_iter_linear_data_not_ptr(env : &mut TestEnv) {
    ...
    // NOTE: the test module cannot import the 'core' library, so the
    // 'core::iterable::Linear' reference does not resolve.
    expect_compile_error(env, "linear_data_not_ptr", ch, "unresolved")
}
...
func neg_iter_chunked_wrong_return(env : &mut TestEnv) {
    ...
    // NOTE: the test module cannot import the 'core' library, so the
    // 'core::iterable::Chunked' reference does not resolve.
    expect_compile_error(env, "chunked_wrong_return", ch, "unresolved")
}
