 ...
 */
class GenericInterfaceDecl : public GenericMembersDecl {
public:

    /**
     * finalize the signature of the struct decl, it means copy the signature in place
     */
    static void finalize_signature(ASTAllocator& allocator, InterfaceDefinition* def);

    /**
     * finalize the body of the struct decl, it means copy the body in place
     */
    static void finalize_body(ASTAllocator& allocator, InterfaceDefinition* def);

    /**
     * master implementation is the first implementation we encounter
     */
    InterfaceDefinition* master_impl;
     ...
     */
    std::vector<InterfaceDefinition*> instantiations;
    ...
    ) : GenericMembersDecl(ASTNodeKind::GenericInterfaceDecl, parent_node, location), master_impl(master_impl) {
     ...
     */
    InterfaceDefinition* register_generic_args(GenericInstantiatorAPI& instantiator, std::vector<BaseType*>& types);
